  ...
  async create(req, res) {
    try {
      const EventSchema = import_zod.z.object({
        ...
        startTime: import_zod.z.string().regex(/^(\d{2}:\d{2}:\d{2})|(\d{2}:\d{2})$/, "O formato Hora esperado \xE9 HH:mm ou HH:mm:ss"),
        endTime: import_zod.z.string().regex(/^(\d{2}:\d{2}:\d{2})|(\d{2}:\d{2})$/, "O formato Hora esperado \xE9 HH:mm ou HH:mm:ss"),
        codPayment: import_zod.z.number()
      });
